        ...
        for i, value in enumerate(windowed_data):
            if value[0] != -1 and prior_value == -1:
                ...
                if len(processed_data) == 0:
                    processed_data = numpy.array([first_timestamp, windowed_steps, window_length])
                else:
                    processed_data = numpy.vstack((processed_data, [first_timestamp, windowed_steps, window_length]))
                prior_value = value[1]
                windowed_steps = 0
            elif value[0] != -1:
                windowed_steps += value[1]
                ...
                prior_value = value[1]
        return processed_data, windowed_data[-1][0]

    def upload_analysis(self, data):
        self.create_table(database_name= self.database_name,
                          table_name   = self.typename,
                          column_names = ['timestamp', 'steps', 'window_length'],
                          column_types = ['VARCHAR(100) NOT NULL PRIMARY KEY', 'INT UNSIGNED', 'INT UNSIGNED'])
        for i in data:
